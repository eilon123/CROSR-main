...
def weibull_tailfitting(meanfiles_path, distancefiles_path,
    ...
    for filename in os.listdir(meanfiles_path):
        ...
        distance_scores = np.load(os.path.join(distancefiles_path,category+".npy"),allow_pickle=True)[()][distance_type]
...
def query_weibull(category_name, weibull_model, distance_type = 'eucos'):
    ...
    category_weibull = []
    category_weibull += [weibull_model[str(category_name)]['mean_vec']]
    category_weibull += [weibull_model[str(category_name)]['distances_%s' %distance_type]]
    category_weibull += [weibull_model[str(category_name)]['weibull_model']]
